...
__forceinline__ __device__
void Compression256_3(uint32_t *  M32)
{
	uint32_t Q[32], XL32, XH32;
...
__global__ __launch_bounds__(TPB, 2)
void bmw256_gpu_hash_32_to_hash(uint32_t threads, uint64_t *g_hash)
{
	const uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
		...
		message[14]=0x100;
		Compression256(message);
		Compression256_3(message);
		
		#pragma unroll
		for (int i = 0; i<4; i++) {
			g_hash[(i*threads) + thread] = (MAKE_ULONGLONG(message[(2 * i) + 8], message[(2 * i + 1) + 8]));
		}
	}
}
...
__host__
void bmw256_cpu_hash_32_to_hash(int thr_id, uint32_t threads, uint32_t startNounce, uint64_t *g_hash, int order)
{
	const uint32_t threadsperblock = TPB;
	dim3 grid((threads + threadsperblock - 1) / threadsperblock);
	dim3 block(threadsperblock);

	bmw256_gpu_hash_32_to_hash << <grid, block >> >(threads, g_hash);
